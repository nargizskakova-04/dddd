...
func setPriceRoutes(handler *PriceHandler, router *http.ServeMux) {
	...
	// Highest Price Endpoints with optional period parameter
	router.HandleFunc("GET /prices/highest/{symbol}", handler.GetHighestPrice)                      // Supports ?period={duration}
	router.HandleFunc("GET /prices/highest/{exchange}/{symbol}", handler.GetHighestPriceByExchange) // Supports ?period={duration}

	// Lowest Price Endpoints (TODO: implement period support)
	...
	// Average Price Endpoints (TODO: implement period support)
	...
	// NEW: Period Information Endpoint (for debugging and API documentation)
	router.HandleFunc("GET /prices/period-info", handler.GetPeriodInfo) // Requires ?period={duration}
}
